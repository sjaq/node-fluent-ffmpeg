...
exports = module.exports = function Processor(command) {
  ...
  this.takeScreenshot = function(config, callback) {
	...
	var self = this,
	    input = Meta.escapedPath(this.options.inputfile),
	    output = Meta.escapedPath(config.outputfile);
	
	self._prepare(function(err, meta) {
		if(err){
			callback(null, err);
		} else {
			if (meta.durationsec && meta.durationsec > parseInt(config.offset, 10)) {
				
				// build screenshot command
				tnArgs = [ 
				  '-ss', (config.offset || meta.durationsec * 0.1),
			      '-i', input,
			      '-vcodec', 'mjpeg',
			      '-vframes', '1',
			      '-an',
				...
				callback(new Error("offset is further than duration of video"));
			}
		}
	});
          ...
          async.until(
            ...
            function(taskcallback) {
              ...
              if (self.options._nice.level) {
                ...
                exec('nice --adjustment="' + self.options._nice.level + '" '+ (process.env.FFMPEG_PATH || 'ffmpeg') + ' ' + tnArgs.join(' '), taskcallback);
              } else {
